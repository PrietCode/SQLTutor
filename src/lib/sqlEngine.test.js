...
test('filters, groups and aggregates follow logical SQL order', () => {
  const execution = executeSql(
    `SELECT category_id, COUNT(*) AS products, ROUND(AVG(price), 2) AS average
    FROM Products
    WHERE stock > 0
  ...
  assert.deepEqual(
    execution.steps.map((item) => item.type),
...
test('joins expose both sources and preserve unmatched rows', () => {
  ...
  assert.deepEqual(
    execution.steps.slice(0, 3).map((item) => item.type),
    ['FROM', 'SOURCE', 'JOIN']
  );
  assert.equal(execution.result.length, 6);
  assert.ok(execution.result.some((row) => row.order_id === null));
  assert.match(
    execution.steps.find((step) => step.type === 'JOIN').detail,
    /\d+ x \d+ = \d+ pares/
  );
  ...
  assert.deepEqual(join.compare.joinKeys, [
    {
      left: 'c.customer_id',
      right: 'o.customer_id',
      leftColumn: 'customer_id',
      rightColumn: 'customer_id',
      label: 'c.customer_id = o.customer_id',
    },
  ]);
});
...
test('scalar functions and SQL Server pagination are evaluated', () => {
  const functions = executeSql(
    `SELECT CONCAT(UPPER(last_name), ', ', first_name) AS name,
...
test('INSERT supports multiple VALUES rows and stops at the first statement', () => {
  const created = executeSql(
    'CREATE TABLE CIUDADES (Id INT, Nombre VARCHAR(80), Provincia INT);',
    createSeedDatabase()
  );
  const inserted = executeSql(
  ...
  const onlyFirst = executeSql(
    `INSERT INTO CIUDADES (Id, Nombre, Provincia) VALUES (8,'Primera',4);
    INSERT INTO CIUDADES (Id, Nombre, Provincia) VALUES (9,'Segunda',4);`,
    inserted.db
  );
...
test('INSERT without explicit columns uses table schema order', () => {
  ...
  const inserted = executeSql(
    ...
    (101,'Raul','Gomez','1992-10-20',100);`,
    db
  );

  assert.equal(inserted.db.EMPLEADOS.length, 2);
  assert.deepEqual(inserted.db.EMPLEADOS[0], {
    Legajo: 100,
    Nombre: 'Juan',
    Apellido: 'Perez',
  ...
  assert.deepEqual(inserted.db.EMPLEADOS[1], {
    Legajo: 101,
    Nombre: 'Raul',
    Apellido: 'Gomez',
...
test('DDL modifies tables, columns and views in the temporary database', () => {
  ...
  const view = executeSql(
    `CREATE VIEW CompletedOrders AS SELECT order_id, total FROM Orders WHERE status = 'completed';`,
    db
  );
...
test('CREATE TABLE preserves many columns and table-level foreign keys', () => {
  ...
  assert.deepEqual(db.CIUDADES.columns, [
    'Id',
    'Nombre',
    'Provincia',
    'CodigoPostal',
  ...
  assert.ok(
    db.CIUDADES.constraints.find((c) => c.type === 'NOT NULL' && c.columns[0] === 'Nombre')
  );
  assert.ok(
    db.CIUDADES.constraints.find(
      (c) => c.type === 'FOREIGN KEY' && c.references.table === 'PROVINCIAS'
    )
  );
...
test('INSERT rejects duplicate PRIMARY KEY values with SQL-Server-style error', () => {
  ...
  assert.throws(
    () => executeSql("INSERT INTO PROVINCIAS (Id, Nombre) VALUES (1,'Mendoza');", db),
    /Infraccion de la restriccion PRIMARY KEY/
  );
  assert.throws(
    () => executeSql("INSERT INTO PROVINCIAS (Id, Nombre) VALUES (1,'Buenos Aires');", db),
    /clave duplicada.*\(1\)/
  );
...
test('INSERT catches duplicate PK across multiple VALUES rows', () => {
  ...
  assert.throws(
    () => executeSql('INSERT INTO TEST (Id, Val) VALUES (1,10), (1,20);', db),
    /clave duplicada.*\(1\)/
  );
  assert.throws(
    () => executeSql('INSERT INTO TEST (Id, Val) VALUES (2,10), (1,20), (1,30);', db),
    /clave duplicada.*\(1\)/
  );
});
...
test('INSERT rejects NULL value in PRIMARY KEY column', () => {
  ...
  assert.throws(
    () => executeSql("INSERT INTO PROVINCIAS (Id, Nombre) VALUES (NULL,'Cordoba');", db),
    /NULL.*columna 'Id'/
  );
  assert.throws(
    () => executeSql("INSERT INTO PROVINCIAS (Nombre) VALUES ('Cordoba');", db),
    /NULL.*columna 'Id'/
  );
});
...
test('INSERT rejects FOREIGN KEY violation when referenced value does not exist', () => {
  ...
  assert.throws(
    () =>
      executeSql("INSERT INTO CIUDADES (Id, Nombre, Provincia) VALUES (1,'Carlos Paz',99);", db),
    /conflicto con la restriccion FOREIGN KEY/
  );
  assert.throws(
    () =>
      executeSql("INSERT INTO CIUDADES (Id, Nombre, Provincia) VALUES (1,'Carlos Paz',99);", db),
    /conflicto ocurrio en la tabla 'dbo.CIUDADES'/
  );
  db = executeSql("INSERT INTO PROVINCIAS (Id, Nombre) VALUES (1,'Cordoba');", db).db;
  assert.doesNotThrow(() =>
    executeSql("INSERT INTO CIUDADES (Id, Nombre, Provincia) VALUES (1,'Carlos Paz',1);", db)
  );
});
...
test('INSERT with self-referencing FK allows forward references within same VALUES', () => {
  ...
  const result = executeSql(
    "INSERT INTO EMPLEADOS VALUES (101,'Raul',100), (102,'Maria',100), (103,'Carlos',100), (104,'Lucia',101), (105,'Pedro',101);",
    db
  );
...
test('INSERT with self-referencing FK correctly rejects invalid forward reference', () => {
  ...
  db = executeSql("INSERT INTO EMPLEADOS VALUES (100,'Juan',NULL);", db).db;
  assert.throws(
    () => executeSql("INSERT INTO EMPLEADOS VALUES (101,'Raul',200);", db),
    /conflicto con la restriccion FOREIGN KEY/
  );
});
...
test('IN subquery in WHERE', () => {
  const result = executeSql(
    `SELECT * FROM Products WHERE category_id IN (SELECT category_id FROM Categories WHERE name LIKE 'E%')`,
    createSeedDatabase()
  );
...
test('>= ALL subquery in WHERE', () => {
  const result = executeSql(
    `SELECT * FROM Employees WHERE salary >= ALL (SELECT salary FROM Employees WHERE department = 'Ventas')`,
    createSeedDatabase()
  );
...
test('>= ALL subquery in HAVING', () => {
  const result = executeSql(
    `SELECT department, AVG(salary) AS avg_salary FROM Employees GROUP BY department HAVING AVG(salary) >= ALL (SELECT AVG(salary) FROM Employees GROUP BY department)`,
    createSeedDatabase()
  );
  assert.equal(result.result.length, 1);
...
test('nested subqueries', () => {
  const result = executeSql(
    `SELECT * FROM Products WHERE category_id IN (SELECT category_id FROM Categories WHERE category_id IN (SELECT DISTINCT category_id FROM Products WHERE stock > 0))`,
    createSeedDatabase()
  );
...
test('DISTINCT runs after SELECT and before ORDER BY', () => {
  ...
  assert.deepEqual(
    result.result.map((row) => row.city),
...
test('HAVING explains aggregate filters and marks removed groups', () => {
  ...
  assert.ok(
    having.compare.beforeRows.some(
      (row) => row.Descrip === 'Transferencia' && row['COUNT(*)'] === 1
    )
  );
  assert.equal(
    result.steps.find((step) => step.type === 'JOIN').compare.joinKeys[0].label,
    'Cod = Cod_forma_pago'
  );
});
...
test('step rows preserve all available rows for visual expansion', () => {
  let db = executeSql(
    'CREATE TABLE TICKETS (Id INT PRIMARY KEY, Fecha DATE, Monto DECIMAL(10,2));',
    createSeedDatabase()
  ).db;
  const values = Array.from(
    { length: 15 },
    (_, index) => `(${index + 1},'2022-01-${String(index + 1).padStart(2, '0')}',${index + 1})`
  ).join(',');
  db = executeSql(`INSERT INTO TICKETS VALUES ${values};`, db).db;
  const result = executeSql(
    "SELECT COUNT(*) AS \"Cantidad Tickets\" FROM TICKETS WHERE Fecha >= '1/1/2022' AND Fecha < '1/2/2022';",
    db
  );
...
test('LIKE implements SQL wildcards and preserves FROM before WHERE filtering', () => {
  ...
  const result = executeSqlScript(script, createSeedDatabase());
  assert.deepEqual(
    result.result.map((row) => row.Modelo),
    ['XR1', 'XR12', 'AXRZ']
  );
  ...
  assert.deepEqual(
    oneChar.result.map((row) => row.Modelo),
    ['XR1']
  );

  const noMatches = executeSql(
    "SELECT Modelo FROM MODELOS_TEST WHERE Modelo LIKE '%ZZ%';",
    result.db
  );
...
test('NULL comparisons follow SQL three-valued logic', () => {
  const db = createSeedDatabase();
  assert.equal(
    executeSql('SELECT first_name FROM Customers WHERE email = NULL;', db).result.length,
    0
  );
  assert.equal(
    executeSql('SELECT first_name FROM Customers WHERE email != NULL;', db).result.length,
  ...
  assert.equal(
    executeSql('SELECT first_name FROM Customers WHERE email IN (NULL);', db).result.length,
    0
  );
  assert.equal(
    executeSql('SELECT first_name FROM Customers WHERE email NOT IN (NULL);', db).result.length,
    0
  );
  assert.equal(
    executeSql('SELECT first_name FROM Customers WHERE email IS NULL;', db).result.length,
    2
  );
});

test('set operators enforce union compatibility and treat NULL duplicates as equal', () => {
  const db = executeSqlScript(
    `CREATE TABLE EMPLEADOS_SET (Dni INT, Apellido VARCHAR(40));
  ...
  assert.throws(
    () => executeSql('SELECT Val FROM SET_A UNION SELECT Val FROM SET_TEXT;', db),
    /Union incompatible/i
  );
});

test('date and conversion scalar functions work in SELECT, WHERE and DML assignments', () => {
  const db = createSeedDatabase();
  const now = executeSql('SELECT GETDATE() AS ahora FROM Customers WHERE customer_id = 1;', db)
    .result[0].ahora;
  ...
  const castDate = executeSql(
    "SELECT order_id FROM Orders WHERE order_date >= CAST('2026-03-01' AS DATE);",
    db
  );
  assert.deepEqual(
    castDate.result.map((row) => row.order_id),
    [104, 105]
  );

  const convertDate = executeSql(
    "SELECT order_id FROM Orders WHERE order_date >= CONVERT(DATE, '2026-03-01');",
    db
  );
  assert.deepEqual(
    convertDate.result.map((row) => row.order_id),
    [104, 105]
  );

  const arithmetic = executeSql(
    'SELECT name, stock + 1 AS next_stock FROM Products WHERE product_id = 6;',
    db
  );
...
test('EXISTS, NOT EXISTS, ANY and SOME subqueries follow WHERE three-valued logic', () => {
  const db = executeSqlScript(
    `CREATE TABLE NULL_COMPARE (Id INT, Val INT);
    INSERT INTO NULL_COMPARE VALUES (1,1), (2,NULL);`,
    createSeedDatabase()
  ).db;

  const exists = executeSql(
  ...
  assert.deepEqual(
    exists.result.map((row) => row.first_name),
    ['Ana', 'Bruno', 'Carla', 'Diego']
  );

  const notExists = executeSql(
    `SELECT first_name FROM Customers c
  ...
  const some = executeSql(
    'SELECT name FROM Products WHERE price < SOME (SELECT price FROM Products WHERE category_id = 1);',
    db
  );
  assert.equal(some.result.length, 5);
  assert.ok(some.result.every((row) => row.name !== 'Laptop Pro'));

  const unknown = executeSql(
    'SELECT Id FROM NULL_COMPARE WHERE Val > ANY (SELECT Val FROM NULL_COMPARE WHERE Id = 2);',
    db
  );
...
test('subquery visual steps preserve filter and projection comparisons', () => {
  const result = executeSql(
    `SELECT name
    FROM Products
    WHERE category_id IN (SELECT category_id FROM Products WHERE stock = 0);`,
    createSeedDatabase()
  );
...
test('non-correlated HAVING subqueries are evaluated once and returned as a fixed set', () => {
  ...
  const having = result.steps.find((step) => step.type === 'HAVING');
  assert.deepEqual(
    having.compare.subquerySteps.map((step) => step.type),
  ...
  assert.deepEqual(
    having.compare.subqueryResults[0].values.sort((a, b) => a - b),
    [985000, 1350000]
  );
});
...
test('correlated EXISTS subqueries record an iteration per outer row with parameters', () => {
  ...
  assert.deepEqual(where.compare.subqueryResults[0].parameters, [
    { name: 'c.customer_id', value: 1 },
  ]);
  assert.deepEqual(where.compare.subqueryResults[0].conditionValues, [
    { name: 'o.customer_id', value: 1 },
    { name: 'c.customer_id', value: 1 },
  ]);
  ...
  const withStatus = executeSql(
    `SELECT first_name
    FROM Customers c
    WHERE EXISTS (SELECT * FROM Orders o WHERE o.customer_id = c.customer_id AND o.status = 'completed');`,
    createSeedDatabase()
  );
  const statusValues = withStatus.steps.find((step) => step.type === 'WHERE').compare
    .subqueryResults[0].conditionValues;
  assert.deepEqual(statusValues, [
    { name: 'o.customer_id', value: 1 },
    { name: 'c.customer_id', value: 1 },
    { name: 'o.status', value: 'completed' },
  ]);
});
...
test('pedagogical validation errors match processing rules', () => {
  const db = createSeedDatabase();
  assert.throws(
    () => executeSql('SELECT name FROM Employees WHERE AVG(salary) > 50000;', db),
    /NUNCA va una función sumaria en el WHERE/
  );
  assert.throws(
    () =>
      executeSql('SELECT name, department, SUM(salary) FROM Employees GROUP BY department;', db),
    /Las columnas del SELECT sin función de grupo DEBEN estar en el GROUP BY/
  );
  assert.throws(
    () =>
      executeSql(
        'SELECT * FROM Products WHERE category_id IN (SELECT category_id FROM Categories ORDER BY category_id);',
        db
      ),
    /subconsulta no puede contener la cláusula ORDER BY/i
  );
  assert.throws(
    () =>
      executeSql(
        'SELECT * FROM Products WHERE category_id IN (SELECT category_id FROM Categories UNION SELECT category_id FROM Products);',
        db
      ),
    /No puede haber UNION de otros SELECT en una subconsulta/
  );
  assert.throws(
    () =>
      executeSql(
        'SELECT c.first_name FROM Customers c JOIN Orders o ON customer_id = customer_id;',
        db
      ),
    /Referencia de columna ambigua/
  );
  assert.throws(
    () => executeSql("INSERT INTO Categories (category_id, name) VALUES (NULL, 'Books');", db),
    /La PK no puede asumir valor nulo/
  );
});
...
test('subquery theoretical restrictions return classroom-oriented errors', () => {
  ...
  assert.throws(
    () =>
      executeSql('SELECT * FROM Employees WHERE (SELECT AVG(salary) FROM Employees) < salary;', db),
    /La subconsulta debe aparecer a la derecha del operador/
  );
  assert.throws(
    () =>
      executeSql(
        'SELECT * FROM Employees WHERE employee_id = (SELECT employee_id, name FROM Employees WHERE employee_id = 1);',
        db
      ),
    /La subconsulta debe devolver una única columna para este operador/
  );
  assert.throws(
    () => executeSql('SELECT * FROM Employees WHERE salary = (SELECT salary FROM Employees);', db),
    /La subconsulta debe devolver una única fila para este operador/
  );
  assert.throws(
    () => executeSql('SELECT * FROM Products WHERE price > SELECT AVG(price) FROM Products;', db),
    /La subconsulta debe estar encerrada entre paréntesis/
  );
  assert.throws(
    () =>
      executeSql(
        'SELECT name, (SELECT department FROM Employees e2 WHERE e2.employee_id = e.employee_id) FROM Employees e;',
        db
      ),
    /No se permiten subconsultas en la lista del SELECT/
  );
  assert.throws(
    () => executeSql('SELECT * FROM Categories WHERE EXISTS (SELECT * FROM Products);', db),
    /EXISTS debe incluir una referencia externa/
  );
});

test('NOT NULL columns reject NULL inserts with a clear integrity error', () => {
  const db = executeSql(
    'CREATE TABLE NN_TEST (Id INT PRIMARY KEY, Nombre VARCHAR(40) NOT NULL);',
    createSeedDatabase()
  ).db;
  assert.throws(
    () => executeSql('INSERT INTO NN_TEST (Id, Nombre) VALUES (1, NULL);', db),
    /NOT NULL.*valor nulo/i
  );
});
...
test('multi-table queries reject ambiguous or implicit joins', () => {
  ...
  assert.throws(
    () => executeSql('SELECT * FROM Customers JOIN Orders;', db),
    /condicion explicita con ON/
  );
});

test('RIGHT JOIN preserves unmatched right rows with NULL left columns', () => {
  let db = executeSql(
    'CREATE TABLE L (Id INT PRIMARY KEY, Val VARCHAR(20));',
    createSeedDatabase()
  ).db;
...
test('GROUP BY rejects non-grouped non-aggregate columns', () => {
  assert.throws(
    () =>
      executeSql(
        'SELECT category_id, name FROM Products GROUP BY category_id;',
        createSeedDatabase()
      ),
    /GROUP BY/
  );
  assert.throws(
    () => executeSql('SELECT category_id, COUNT(*) AS total FROM Products;', createSeedDatabase()),
    /funcion de agregado/
  );
});
...
test('seed database enforces PK and FK constraints', () => {
  ...
  assert.throws(
    () =>
      executeSql(
        "INSERT INTO Orders (order_id, customer_id, total, status, order_date) VALUES (101, 1, 10, 'x', '2026-01-01');",
        db
      ),
    /PRIMARY KEY/
  );
  assert.throws(
    () =>
      executeSql(
        "INSERT INTO Orders (order_id, customer_id, total, status, order_date) VALUES (200, 999, 10, 'x', '2026-01-01');",
        db
      ),
    /FOREIGN KEY/
  );
...
test('DML and DDL reject unknown columns and invalid foreign keys', () => {
  assert.throws(
    () => executeSql('INSERT INTO Products (missing) VALUES (1);', createSeedDatabase()),
    /no existe/
  );
  assert.throws(
    () => executeSql('UPDATE Products SET missing = 1;', createSeedDatabase()),
    /no existe/
  );
  assert.throws(
    () =>
      executeSql(
        'CREATE TABLE BadChild (Id INT PRIMARY KEY, ParentId INT, FOREIGN KEY (ParentId) REFERENCES Missing(Id));',
        createSeedDatabase()
      ),
    /referenciada/
  );
  assert.throws(
    () =>
      executeSql(
        'CREATE TABLE BadRef (Id INT PRIMARY KEY, ParentName VARCHAR(80), FOREIGN KEY (ParentName) REFERENCES Categories(name));',
        createSeedDatabase()
      ),
...
test('CREATE TABLE rejects misspelled keywords and missing commas', () => {
  ...
  assert.throws(
    () => executeSql('CREATE TABLE BadPk (Id INT PRIMARY KKEY);', db),
    /KKEY|sintaxis/i
  );
  assert.throws(
    () =>
      executeSql(
        'CREATE TABLE BadFk (Id INT PRIMARY KEY, ParentId INT, FOREIGN KEY (ParentId) REFRENCES PADRES(Id));',
        db
      ),
    /REFRENCES|sintaxis/i
  );
  assert.throws(
    () => executeSql('CREATE TABLE MissingComma (Id INT PRIMARY KEY Nombre VARCHAR(50));', db),
    /Nombre VARCHAR|sintaxis/i
  );
  assert.throws(
    () => executeSql('CREATE TABLE BadType (Id INTT PRIMARY KEY);', db),
    /tipo de dato "INTT"/i
  );
  assert.throws(
    () => executeSql('ALTER TABLE PADRES ADD Extra INTT;', db),
    /tipo de dato "INTT"|variante/i
  );
...
test('FOREIGN KEY structure must match referenced key columns and types', () => {
  let db = executeSql(
    'CREATE TABLE PADRES (Id INT PRIMARY KEY, Codigo VARCHAR(10) UNIQUE);',
    createSeedDatabase()
  ).db;
  assert.throws(
    () =>
      executeSql(
        'CREATE TABLE BadTypeFk (Id INT PRIMARY KEY, ParentId VARCHAR(10), FOREIGN KEY (ParentId) REFERENCES PADRES(Id));',
        db
      ),
    /no coincide en tipos/i
  );
  db = executeSql(
    'CREATE TABLE PADRES_COMP (Modelo VARCHAR(100), Marca VARCHAR(100), PRIMARY KEY (Modelo, Marca));',
    db
  ).db;
  assert.throws(
    () =>
      executeSql(
        'CREATE TABLE BadCountFk (Id INT PRIMARY KEY, Modelo VARCHAR(100), FOREIGN KEY (Modelo) REFERENCES PADRES_COMP(Modelo, Marca));',
        db
      ),
    /misma cantidad/i
  );
  assert.doesNotThrow(() =>
    executeSql(
      'CREATE TABLE GoodCompositeFk (Modelo VARCHAR(100), Marca VARCHAR(100), Item INT PRIMARY KEY, FOREIGN KEY (Modelo, Marca) REFERENCES PADRES_COMP(Modelo, Marca));',
      db
    )
  );
});
...
test('composite PRIMARY KEY enforces uniqueness and non-null components', () => {
  let db = executeSql(
    'CREATE TABLE CARACTxMOTO (Modelo VARCHAR(100), Marca VARCHAR(100), Cod_caract INT, Valor VARCHAR(40), PRIMARY KEY (Modelo, Marca, Cod_caract));',
    createSeedDatabase()
  ).db;
  db = executeSql("INSERT INTO CARACTxMOTO VALUES ('Wave','Honda',1,'125cc');", db).db;
  assert.throws(
    () => executeSql("INSERT INTO CARACTxMOTO VALUES ('Wave','Honda',1,'Repetida');", db),
    /PRIMARY KEY|duplicada/i
  );
  assert.throws(
    () => executeSql("INSERT INTO CARACTxMOTO VALUES ('Wave',NULL,2,'Sin marca');", db),
    /NULL.*Marca/i
  );
});

...
export const createSeedDatabase = () => ({
  ...
  Categories: table(
    ...
    ['category_id', 'name'],
    { category_id: 'INT', name: 'VARCHAR(80)' },
    [{ type: 'PRIMARY KEY', columns: ['category_id'] }]
  ),
  Products: table(
    [
    ...
    ['product_id', 'name', 'price', 'category_id', 'stock'],
    {
      product_id: 'INT',
      name: 'VARCHAR(120)',
      price: 'DECIMAL(10,2)',
  ...
  CustomerImports: table(
    ...
    ['import_id', 'customer_id', 'source'],
    { import_id: 'INT', customer_id: 'INT', source: 'VARCHAR(40)' },
    [{ type: 'PRIMARY KEY', columns: ['import_id'] }]
  ),
  Employees: table(
    [
      {
    ...
    [{ type: 'PRIMARY KEY', columns: ['employee_id'] }]
  ),
});

...
export function useSqlRuntime() {
  ...
  const executeQuery = (sql) => {
    try {
      if (!hasTerminatingSemicolon(sql))
        throw new Error('La sentencia SQL debe finalizar con punto y coma (;).');
      const statements = splitSqlStatements(sql);
      const next =
        statements.length > 1
          ? executeSqlScript(sql, databaseRef.current)
          : executeSql(sql, databaseRef.current);
  ...
  const executeScript = (content) => {
    if (!hasTerminatingSemicolon(content))
      throw new Error('El archivo SQL debe finalizar cada sentencia con punto y coma (;).');

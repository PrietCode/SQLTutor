...
export function TableDetailModal({ tableName, rows, onClose }) {
  ...
  const foreignKeys = (rows.constraints || []).filter(
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={`Detalle de tabla ${tableName}`}
      tabIndex={-1}
    >
      <div className="table-detail-modal">
        ...
        <div className="detail-header">
          <div>
            <span className="eyebrow">TABLA DEL SANDBOX</span>
            <h2>
              <Icon name="table" /> {tableName}
            </h2>
          </div>
        ...
        <div className="detail-grid">
              ...
              <div className="detail-columns">
                {columns.length ? (
                  columns.map((column, index) => (
                    <div key={column} className="detail-column-row">
                      <span className={index === 0 ? 'key-dot' : 'column-dot'}>
                        {index === 0 ? 'PK' : ''}
                      </span>
                      <strong>{column}</strong>
                      <em>{columnTypeLabel(rows, column)}</em>
                    </div>
                  ))
                ) : (
                  <p className="muted">Sin columnas definidas.</p>
                )}
              </div>
            </section>
            <section className="detail-card">
              ...
              {foreignKeys.length ? (
                <div className="fk-list">
                  {foreignKeys.map((fk, index) => (
                    <div className="fk-row" key={index}>
                      <strong>{fk.columns.join(', ')}</strong>
                      <span>
                        es clave foránea de <b>{fk.references.table}</b> (
                ...
                <p className="muted">No hay claves foráneas declaradas para esta tabla.</p>
              )}
            </section>
          </div>
          <section className="detail-card records-detail-card">
            ...
            {columns.length ? (
                ...
                <table>
                  ...
                  <tbody>
                    {rows.length ? (
                      ...
                      <tr>
                        <td colSpan={columns.length}>Tabla vacía: todavía no hay registros.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
              ...
              <div className="no-records-note">No hay columnas para mostrar.</div>
            )}
          </section>
        </div>
      </div>

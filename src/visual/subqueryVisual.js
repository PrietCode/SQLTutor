const subqueryGroupKey = (summary, index) =>
  summary.mode === 'correlated'
...
export function buildSubqueryGroups(compare) {
  ...
  const summaries = compare.subqueryResults?.length
    ? compare.subqueryResults
    : [
        {
          id: 'legacy',
          mode: 'uncorrelated',
          operator: 'SUBCONSULTA',
          rowCount: steps.at(-1)?.count || 0,
          values: [],
        },
      ];
  ...
  summaries.forEach((summary, index) => {
    ...
    if (!byKey.has(key)) {
      const group = {
        ...
        steps,
        traced,
        externalRows: compare.beforeRows || [],
        mode: summary.mode,
        order: groups.length,
      };
      byKey.set(key, group);
  ...
  for (let index = 0; index < condition.length; index += 1) {
    ...
    if (char === "'") {
      ...
      while (end < condition.length) {
        if (condition[end] === "'" && condition[end + 1] === "'") {
          end += 2;
          continue;
        }
        if (condition[end] === "'") {
...
export const parentConditionText = (step) =>
  hideSubquerySql((step.detail || '').split(/\.\s+Se conservan/i)[0]);
...
export const formatSubqueryValue = (value) =>
  value == null ? 'NULL' : typeof value === 'string' ? `'${value}'` : String(value);

export const conditionWithValues = (summary, condition) => {
  if (!summary.conditionValues?.length) return condition;
  const values = new Map(
    summary.conditionValues.map((item) => [item.name.toLowerCase(), item.value])
  );
  return condition.replace(/\b[A-Za-z_]\w*\.[A-Za-z_]\w*\b/g, (identifier) =>
    values.has(identifier.toLowerCase())
      ? `${identifier} (${formatSubqueryValue(values.get(identifier.toLowerCase()))})`
      : identifier
  );
};

export const subqueryReturnText = (summary) => {
  if (/EXISTS/i.test(summary.operator || ''))
    return `${summary.operator}: ${summary.verdict || (summary.rowCount > 0 ? 'TRUE' : 'FALSE')} (${summary.rowCount} filas)`;
  if (!summary.values?.length) return 'conjunto vacío';
  const values = summary.values.map(formatSubqueryValue).join(', ');
  return summary.rowCount === 1
    ? `valor ${values}`
    : `conjunto { ${values}${summary.rowCount > summary.values.length ? ', ...' : ''} }`;
};
...
export const subqueryConditionText = (summary) => {
  ...
  if (/EXISTS/i.test(summary.operator || ''))
    return `¿Existe alguna fila que cumpla ${annotatedInner}?`;
  if (/ANY|SOME|ALL/i.test(summary.operator || ''))
    return `Se compara el valor externo contra el conjunto devuelto por la subconsulta usando ${summary.operator}.`;
  if (/IN/i.test(summary.operator || ''))
    return `Se verifica pertenencia contra la columna devuelta por la subconsulta.`;
